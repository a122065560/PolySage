...
hiddenimports = ['PyQt6', 'PyQt6.QtCore', 'PyQt6.QtGui', 'PyQt6.QtWidgets', 'PyQt6.sip',
                 'qasync', 'playwright', 'playwright.async_api', 'playwright._impl',
                 'openai', 'platform_adapter', 'macos_adapter', 'windows_adapter']
...
filtered_datas = []
for d in tmp_ret[0]:
    if 'chromium' in str(d[0]).lower() and 'driver' not in str(d[0]).lower():
        continue
    if 'ffmpeg' in str(d[0]).lower() and 'driver' not in str(d[0]).lower():
        continue
...
exclude_list = [
    'PyQt6.Qt6', 'tkinter', 'matplotlib', 'numpy', 'pandas', 'PyQt5', 'PySide6',
    ...
    'PyQt6.QtMultimediaWidgets', 'PyQt6.QtNetwork', 'PyQt6.QtNfc',
    ...
    'PyQt6.QtQuick3D', 'PyQt6.QtQuickControls2', 'PyQt6.QtQuickWidgets',
...
a = Analysis(
    ['main.py', 'ui_main_window.py', 'ui_widgets.py', 'ui_worker.py', 'ui_flowlayout.py',
     'browser.py', 'core.py', 'config_manager.py', 'utils.py', 'logger.py',
     'platform_adapter.py', 'macos_adapter.py', 'windows_adapter.py'],
    pathex=[], binaries=binaries, datas=datas, hiddenimports=hiddenimports,
    hookspath=[], hooksconfig={}, runtime_hooks=[], excludes=exclude_list, noarchive=False,
)
pyz = PYZ(a.pure)
exe_kwargs = dict(pyz=pyz, scripts=a.scripts, exclude_binaries=True, name='PolySage',
...
exe = EXE(**exe_kwargs)
coll = COLLECT(exe, a.binaries, a.datas, strip=False, upx=True, upx_exclude=[], name='PolySage')
if IS_MACOS:
    app = BUNDLE(coll, name='PolySage.app',
